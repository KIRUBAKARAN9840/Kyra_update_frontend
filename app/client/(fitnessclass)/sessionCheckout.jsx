...
const SessionCheckout = () => {
  ...
  // selectedDates is JSON array of YYYY-MM-DD strings
  ...
  const fetchDetails = useCallback(async () => {
    if (!gymId || !sessionId || !scheduleId || selectedDates.length === 0)
      return;
    setLoading(true);

    try {
      const res = await getPaymentDetailSession(
        gymId,
        sessionId,
        scheduleId,
        selectedDates,
  ...
  }, [gymId, sessionId, scheduleId, selectedDates]);
          ...
          {selectedDates.map((iso, i) => (
            <View key={i} style={styles.sessionRow}>
              <Text style={styles.sessionLabel}>
                {sessionName} · {formatDateChip(iso)}
              </Text>
