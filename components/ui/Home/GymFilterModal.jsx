...
const GymFilterModal = ({
  ...
  const [tempFilters, setTempFilters] = useState({
    ...
    has_daily_pass: filters.has_daily_pass || false,
    selected_fitness_types: filters.selected_fitness_types || [],
    sessions: filters.sessions || false,
    selected_sessions: filters.selected_sessions || [],
  ...
  const handleFilterFieldSelect = (field) => {
    // Just close modal to show location selector, don't call API yet
    onClose();
    setTimeout(() => {
      if (onApply) {
        onApply({ ...tempFilters, _pendingField: field });
      }
    }, 300);
  };

  const handleFitnessTypeToggle = (fitnessTypeId) => {
    // Just update local state, don't call API yet
    setTempFilters((prev) => {
      const newSelected = prev.selected_fitness_types.includes(fitnessTypeId)
        ? prev.selected_fitness_types.filter((id) => id !== fitnessTypeId)
        : [...prev.selected_fitness_types, fitnessTypeId];
      return {
        ...prev,
        selected_fitness_types: newSelected,
      };
    });
  };
  ...
  const handleSessionToggle = (sessionId) => {
    // Just update local state, don't call API yet
    setTempFilters((prev) => {
      const newSelected = prev.selected_sessions.includes(sessionId)
        ? prev.selected_sessions.filter((id) => id !== sessionId)
        : [...prev.selected_sessions, sessionId];
      return {
        ...prev,
        selected_sessions: newSelected,
        sessions: newSelected.length > 0,
      };
    });
  };

  const handleMembershipTypeToggle = (membershipId) => {
    setTempFilters((prev) => {
      const newSelected = prev.membership_types.includes(membershipId)
        ? prev.membership_types.filter((id) => id !== membershipId)
        : [...prev.membership_types, membershipId];
      return {
        ...prev,
        membership_types: newSelected,
      };
    });
  };
  ...
  const handleDailyPassToggle = () => {
    // Just update local state, don't call API yet
    setTempFilters((prev) => {
      return {
        ...prev,
        has_daily_pass: !prev.has_daily_pass,
      };
    });
  };
  ...
  const activeFilterCount = useMemo(() => {
    ...
    if (tempFilters.has_daily_pass) count++;
    if (tempFilters.selected_fitness_types.length > 0)
      count += tempFilters.selected_fitness_types.length;
    if (tempFilters.sessions) count += tempFilters.selected_sessions.length;
    if (tempFilters.membership_types.length > 0)
  ...
  const displaySessions = showAllSessions
    ? SESSION_TYPES
    : SESSION_TYPES.slice(0, 6);

  const showSort =
    tempFilters.has_daily_pass ||
    tempFilters.sessions ||
    tempFilters.membership_types.length > 0;
  const sortAfterDaily =
    tempFilters.has_daily_pass &&
    !tempFilters.sessions &&
    tempFilters.membership_types.length === 0;
        ...
        >
          ...
          <View style={styles.locationCard}>
            <FilterRow
              ...
              onPress={() => handleFilterFieldSelect("state")}
            ...
            <FilterRow
              ...
              onPress={() => handleFilterFieldSelect("city")}
            ...
            <FilterRow
              ...
              onPress={() => handleFilterFieldSelect("area")}
            ...
            <FilterRow
              ...
              onPress={() => handleFilterFieldSelect("pincode")}
              onClear={() =>
          ...
          <Pressable
            style={[
              styles.dailyPassCard,
              tempFilters.no_cost_emi && styles.dailyPassCardActive,
            ]}
            onPress={handleNoCostEmiToggle}
          >
            <View style={styles.dailyPassLeft}>
              <Text style={styles.dailyPassTitle}>No Cost EMI</Text>
            ...
            <View
              style={[
                styles.dailyPassCheckbox,
                tempFilters.no_cost_emi && styles.dailyPassCheckboxChecked,
              ]}
            >
              {tempFilters.no_cost_emi && (
          ...
          <Pressable
            style={[
              styles.dailyPassCard,
              tempFilters.has_daily_pass && styles.dailyPassCardActive,
            ]}
            onPress={handleDailyPassToggle}
          >
            <View style={styles.dailyPassLeft}>
              <Text style={styles.dailyPassTitle}>Daily Pass</Text>
              <Text style={styles.dailyPassSubtitle}>
                Search nearby gyms with Daily Pass
              </Text>
            </View>
            <View
              style={[
                styles.dailyPassCheckbox,
                tempFilters.has_daily_pass && styles.dailyPassCheckboxChecked,
              ]}
            >
              {tempFilters.has_daily_pass && (
                <Ionicons name="checkmark" size={16} color="#fff" />
          ...
          {/* Sort By after Daily Pass (only daily pass selected, no sessions) */}
          {showSort && sortAfterDaily && <SortBlock />}

          {/* Sessions Section */}
          <Text style={styles.sectionHeader}>Sessions</Text>
          <Text style={styles.sectionSubtitle}>
            Select the type of Session class
          </Text>

          {/* Sessions Grid */}
          <View style={styles.sessionsGrid}>
            {displaySessions.map((session) => {
              const selected = tempFilters.selected_sessions.includes(
                session.id,
              );
              return (
                <Pressable
            ...
            {!showAllSessions && SESSION_TYPES.length > 6 && (
              <TouchableOpacity
                style={styles.showMoreChip}
                onPress={() => setShowAllSessions(true)}
              >
                <Text style={styles.showMoreChipText}>
                  +{SESSION_TYPES.length - 6}
                </Text>
              </TouchableOpacity>
            )}
          </View>

          {/* Sort By after Sessions (sessions active, no membership selected) */}
          {showSort &&
            tempFilters.sessions &&
            tempFilters.membership_types.length === 0 && <SortBlock />}

          {/* Membership / PT Plans Section */}
          ...
          {/* Sort By after Membership (membership active) */}
          {showSort && tempFilters.membership_types.length > 0 && <SortBlock />}
        </ScrollView>
...
// Filter Row Component (for location fields)
...
const styles = StyleSheet.create({
  ...
  // Show More Chip (inline with other chips)
  showMoreChip: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
